...
async function getConnection() {
  return await mysql.createConnection({
    ...
    password: "Astra125", //Your DB password
  });
}
...
async function makeDatabase(connection) {
  ...
  const [result, _] = await connection.query(
    "CREATE DATABASE IF NOT EXISTS cs317db;"
  );
...
async function makeUsersTable(connection) {
  const [result, _] = await connection.query(
    ...
    `
    CREATE TABLE IF NOT EXISTS cs317db.users (
...
async function makePostsTable(connection) {
  const [result, _] = await connection.query(
    ...
    `
    CREATE TABLE IF NOT EXISTS cs317db.posts (
      id INT NOT NULL AUTO_INCREMENT,
...
async function makeCommentsTable(connection) {
  const [result, _] = await connection.query(
    ...
    ` 
    CREATE TABLE IF NOT EXISTS cs317db.comments (
      id INT NOT NULL AUTO_INCREMENT,
      ...
      PRIMARY KEY (id),
      UNIQUE INDEX id_UNIQUE (id ASC) VISIBLE,
      INDEX comment_author_idx (fk_userid ASC) VISIBLE,
      INDEX comment_belongsTo_idx (fk_posts ASC) VISIBLE,
      ...
      CONSTRAINT comment_belongsTo
        FOREIGN KEY (fk_posts)
        REFERENCES cs317db.posts (id)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION)
    ENGINE = InnoDB;
    `
...
(async function main() {
  ...
  try {
    ...
    //TODO make sure to change yourdbnamehere
    await connection.query("USE cs317db"); // set new DB to the current DB
